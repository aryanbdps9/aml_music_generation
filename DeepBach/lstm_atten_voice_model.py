...
class VoiceModel(nn.Module):
    def __init__(self,
        ...
        self.em_size = note_embedding_dim * self.num_voices + meta_embedding_dim * self.num_metas
        self.lstm_left = nn.LSTM(input_size= self.em_size,
                                 hidden_size=self.em_size,
                                 num_layers=num_layers,
                                 dropout=dropout_lstm,
                                 batch_first=True)
        self.lstm_right = nn.LSTM(input_size=self.em_size,
                                  hidden_size=self.em_size,
                                  num_layers=num_layers,
                                  dropout=dropout_lstm,
                                  batch_first=True)
        ...
        self.mlp_predictions = nn.Sequential(
            nn.Linear((note_embedding_dim * (self.num_voices) 
                       + (meta_embedding_dim * self.num_metas)) * 3,
                      hidden_size_linear),
            nn.ReLU(),
    ...
    def forward(self, *input):
        ...
        # # main part of deep Bach
        hidden = init_hidden(
            num_layers=self.num_layers,
            batch_size=batch_size,
            # lstm_hidden_size=self.lstm_hidden_size,
            lstm_hidden_size=self.em_size,
        )
        ...
        # print("right size" , right.size())
        # print("center size" , center.size())
        # left_context = left[:,-1,:]
        # right_context = right[:,-1,:]                
        ...
        predictions = torch.cat([left_context, center, right_context], 1)

...
def get_c_kernel(n):
    e = 64
    res = conv = nn.Sequential(
        nn.Conv2d(n, 16, kernel_size=(5, 5)), 
        nn.ReLU(),
        nn.MaxPool2d((2, 2), stride=2),  
        nn.Conv2d(16, 32, kernel_size=(5, 5), padding=(1, 1)),
        nn.ReLU(),
        nn.MaxPool2d((2, 2), stride=2),  # 32 X 19 X 5
        nn.Conv2d(32, 64, kernel_size=(3, 3)),  # 64 X 17 X 1
        nn.ReLU(),
        nn.MaxPool2d((2, 2), stride=2),  # 32 X 19 X 5
        # nn.AvgPool2d((4,3), stride=2), # 64 X 17 X 1
        nn.Conv2d(64, n*e, kernel_size=(5, 3), stride=(1, 1)),
        nn.ReLU(),
    )
    return res
...
class VoiceModel(nn.Module):
    ...
    def forward(self, *input):
        ...
        ntsteps = left.size(1)+right.size(1)
